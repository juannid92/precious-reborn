...
const upper = (value: string | null | undefined, fallback: string) =>
  value ? value.toUpperCase() : fallback;
...
function toOptions(
  ...
  return {
    ...
    metal_quality:
      config.metalType === "platinum" ? "PLATINUM" : upper(config.metalQuality, "KT_18"),
    metal_type: upper(config.metalType, "GOLD"),
    mounting_metal_color:
      config.metalType === "platinum" ? "WHITE_GOLD" : upper(config.shankMetalColor, "YELLOW_GOLD"),
    ...
    ring_head_metal_color:
      config.metalType === "platinum" ? "WHITE_GOLD" : upper(config.headMetalColor, "YELLOW_GOLD"),
    ...
    side_stones_type: hasSide
      ? upper(
          config.sideStoneType === "lab_diamond" ? "diamonds" : config.sideStoneType,
          "DIAMONDS",
        )
      : null,
  };
...
export function RingStudioPreview({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      setError(null);
      try {
        const result = await getPreview({ data: { options } });
        if (cancelled) return;
        ...
        if (cancelled) return;
        if (mountingImage)
          setImages({ down: mountingImage, front: mountingImage, side: mountingImage });
        setLive(false);
        setError(
          "Il rendering fotografico si aggiornerà appena il servizio Nivoda sarà disponibile.",
        );
      } finally {
  ...
  }, [getPreview, requestKey, mountingImage]);
  ...
  return (
    <div className="w-full">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <span
          className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-wider ${live ? "border-emerald-400/25 text-emerald-300" : "border-white/15 text-bone/45"}`}
        >
          {loading ? (
      ...
      <div className="grid grid-cols-[64px_minmax(0,1fr)] gap-3 sm:grid-cols-[76px_minmax(0,1fr)]">
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/10 bg-white">
          <img
            ...
            className={`aspect-square h-full w-full object-contain transition-opacity ${loading ? "opacity-55" : "opacity-100"}`}
          />
          {loading && (
            <div className="absolute inset-0 grid place-items-center bg-white/20">
              <Loader2 className="h-7 w-7 animate-spin text-[#1a1a1a]" />
            </div>
          )}
          {mountingImage && !live && (
            <img
              src={mountingImage}
              alt="Montatura selezionata"
              className="absolute bottom-3 right-3 h-16 w-16 rounded-lg border border-black/10 bg-white object-cover shadow-sm"
            />
          )}
        </div>
      </div>
      ...
      <p className="mt-3 text-[10px] leading-relaxed text-bone/35">
        L’immagine è un riferimento digitale. Proporzioni e finiture definitive vengono confermate
        dal maestro orafo.
      </p>
    </div>
  );
